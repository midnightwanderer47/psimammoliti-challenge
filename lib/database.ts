...
export async function bookSession(sessionData: {
  ...
  try {
    ...
    // Get the day of week from the session date
    const sessionDate = new Date(sessionData.session_date)
    const dayOfWeek = sessionDate.getDay()

    // Mark the corresponding available slot as booked
    const { error: slotUpdateError } = await supabase
      .from("available_slots")
      .update({ is_booked: true })
      .eq("psychologist_id", sessionData.psychologist_id)
      .eq("day_of_week", dayOfWeek)
      .eq("time_slot", sessionData.session_time)
      .eq("modality", sessionData.modality)

    if (slotUpdateError) {
      console.error("Error updating slot booking status:", slotUpdateError)
      // Continue with session creation even if slot update fails
    }
